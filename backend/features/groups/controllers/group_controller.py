...
from ..schemas.group_schema import GroupResposne, GroupCreate, GroupUpdate, MembershipCreate, GroupWithRoleResponse, GroupJoin, AdminGroupResponse
...
def create_group(group_create: GroupCreate, db: Session = Depends(get_db),
    ...
    return AdminGroupResponse(name=group.name, description=group.description, id=group.id, role=role, connection_string=group.connection_string)
...
def get_groups(db: Session = Depends(get_db), user_data: UserData = Depends(TokenService.get_user_data)):
    ...
    for group in groups:
        ...
        group_with_role = GroupWithRoleResponse(name=group.name, description=group.description, id=group.id, role=role)
        groups_with_roles.append(group_with_role)

    return groups_with_roles


@router.get("/api/groups/{group_id}", tags=["Groups"], response_model=GroupWithRoleResponse | AdminGroupResponse)
def get_group(user_data: Annotated[UserData, Depends(TokenService.get_user_data)], group_id: int, db: Session = Depends(get_db)):
    group_service = GroupService(db)
    ...
    return GroupWithRoleResponse(name=group.name, description=group.description, id=group.id, role=role)


@router.put("/api/groups/{group_id}", tags=["Groups"], response_model=GroupResposne)
def update_group(user_data: Annotated[UserData, Depends(TokenService.get_user_data)], group_id: int, group: GroupUpdate, db: Session = Depends(get_db)):
    group_service = GroupService(db)
    group_service.check_if_admin(user_data.id, group_id)
    updated_group = group_service.update_group(group_id, group)
    return GroupResposne(name=updated_group.name, description=updated_group.description, id=updated_group.id)


@router.delete("/api/groups/{group_id}", tags=["Groups"], response_model=GroupResposne)
def delete_group(user_data: Annotated[UserData, Depends(TokenService.get_user_data)], group_id: int, db: Session = Depends(get_db)):
    group_service = GroupService(db)
    group_service.check_if_admin(user_data.id, group_id)
    deleted_group = group_service.delete_group(group_id)
    return GroupResposne(name=deleted_group.name, description=deleted_group.description, id=deleted_group.id)


@router.post("/api/groups/{group_id}/add-user/{user_id}", tags=["Groups"], response_model=GroupResposne)
def add_user_to_group(user_data: Annotated[UserData, Depends(TokenService.get_user_data)], group_id: int, user_id: int, role: str, db: Session = Depends(get_db)):
    group_service = GroupService(db)
    group_service.check_if_admin(user_data.id, group_id)
    group = group_service.get_group(group_id)
...
@router.delete("/api/groups/{group_id}/remove-user/{user_id}", tags=["Groups"], response_model=GroupResposne)
def remove_user_from_group(user_data: Annotated[UserData, Depends(TokenService.get_user_data)], group_id: int, user_id: int, db: Session = Depends(get_db)):
    group_service = GroupService(db)
    group_service.check_if_admin(user_data.id, group_id)
...
@router.get("/api/groups/{group_id}/users", tags=["Groups"], response_model=UserListResponse)
def get_users_in_group(group_id: int, db: Session = Depends(get_db)):
    ...
    for user in users:
        response.users.append(UserResponse(user_id=user.id, name=user.name, surname=user.surname))
    return response
